...
def analyze_website(url: str, api_key: str, api_url: str, save_screenshots: bool = False) -> Dict[str, Any]:
    """
...
def main():
    ...
    parser.add_argument("--save-screenshots", action="store_true", help="Save screenshots permanently")
    ...
    if "error" not in results:
        ...
        if results["lighthouse"]["available"]:
            print(f"⚡ Lighthouse Performance: {results['lighthouse']['performanceScore']:.0f}/100")
            print(f"⚡ First Contentful Paint: {results['lighthouse']['fcpSeconds']:.1f}s")
            print(f"⚡ Largest Contentful Paint: {results['lighthouse']['lcpSeconds']:.1f}s")
