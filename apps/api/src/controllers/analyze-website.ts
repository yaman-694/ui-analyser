...
export const analyzeUIController = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    const { url } = req.body;
    ...
    try {
        ...
        pythonProcess.on("close", (code) => {
            ...
            try {
                ...
                const lighthouseMatch = stdoutData.match(/Performance Score: (\d+\.?\d*)\/100/);
                const loadTimeMatch = stdoutData.match(/Load Time: (\d+\.?\d*) seconds/);
                ...
                const analysisResults = {
                    ...
                    performanceScore: lighthouseMatch ? parseFloat(lighthouseMatch[1]) : null,
